...
final class Base{
	...
	private static function parse_conf(&$setting_config){
		$feiwa_config = $GLOBALS['config'];
		if(is_array($feiwa_config['db']['slave']) && !empty($feiwa_config['db']['slave'])){
			$dbslave = $feiwa_config['db']['slave'];
			$sid     = array_rand($dbslave);
			$feiwa_config['db']['slave'] = $dbslave[$sid];
		}else{
			$feiwa_config['db']['slave'] = $feiwa_config['db'][1];
		}
		$feiwa_config['db']['master'] = $feiwa_config['db'][1];
		$setting_config = $feiwa_config;
		$setting = ($setting = rkcache('setting')) ? $setting : rkcache('setting',true);
		$setting['feiwa_version'] = '<span class="vol"><a href="http://www.feiwa.org" target="_blank"><font class="b">Fei</font><font class="o">Wa</font></a></span>';
		$setting_config = array_merge_recursive($setting,$feiwa_config);
	}
	...
	private static function control(){
		...
		//二级域名
		if ($GLOBALS['setting_config']['enabled_subdomain'] == '1' && $_GET['app'] == 'index' && $_GET['feiwa'] == 'index'){
			$store_id = subdomain();
			if ($store_id > 0) $_GET['app'] = 'show_store';
		}
		$app_file = realpath(BASE_PATH.'/control/'.$_GET['app'].'.php');
...
private static function controladmin($admin){
		...
		$app_file = realpath(BASE_PATH.'/modules/'.$admin.'/control/'.$_GET['app'].'.php');
	...
	private static function cp(){
		...
		if (strpos(self::CPURL,'||') !== false){
			...
			header('location: http://www.feiwa.org');exit();
		}else{
