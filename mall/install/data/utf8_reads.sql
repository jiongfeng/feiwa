...
CREATE TABLE `#__reads_article_attitude` (
...
CREATE TABLE `#__reads_article_class` (
...
) ENGINE=InnoDB AUTO_INCREMENT=7 DEFAULT CHARSET=utf8 COMMENT='cms文章分类表';


CREATE TABLE `#__reads_comment` (
...
CREATE TABLE `#__reads_comment_up` (
  ...
  PRIMARY KEY (`up_id`)
) ENGINE=InnoDB AUTO_INCREMENT=5 DEFAULT CHARSET=utf8 COMMENT='CMS评论顶表';


CREATE TABLE `#__reads_index_module` (
...
) ENGINE=InnoDB AUTO_INCREMENT=19 DEFAULT CHARSET=utf8 COMMENT='CMS首页模块表';


CREATE TABLE `#__reads_module` (
  ...
  PRIMARY KEY (`module_id`)
) ENGINE=InnoDB AUTO_INCREMENT=14 DEFAULT CHARSET=utf8 COMMENT='CMS模板模块表';


CREATE TABLE `#__reads_module_assembly` (
...
CREATE TABLE `#__reads_module_frame` (
...
CREATE TABLE `#__reads_navigation` (
...
CREATE TABLE `#__reads_picture_class` (
  `class_id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '分类编号 ',
  `class_name` varchar(50) NOT NULL COMMENT '分类名称',
  `class_sort` tinyint(1) unsigned NOT NULL DEFAULT '255' COMMENT '排序',
  PRIMARY KEY (`class_id`)
...
CREATE TABLE `#__reads_picture_image` (
...
CREATE TABLE `#__reads_tag` (
  ...
  PRIMARY KEY (`tag_id`)
) ENGINE=InnoDB AUTO_INCREMENT=6 DEFAULT CHARSET=utf8 COMMENT='CMS标签表';


CREATE TABLE `#__reads_tag_relation` (
...
) ENGINE=InnoDB AUTO_INCREMENT=4 DEFAULT CHARSET=utf8 COMMENT='CMS标签关系表';

INSERT INTO `#__reads_module` (`module_id`, `module_title`, `module_name`, `module_type`, `module_class`) VALUES
(1, '综合模块', 'index', 'index', 1),
(2, '分享秀模块', 'micro', 'micro', 1),
...
INSERT INTO `#__reads_module_assembly` (`assembly_id`, `assembly_title`, `assembly_name`, `assembly_explain`) VALUES
(1, '文章', 'article', '文章组件'),
(2, '图片', 'picture', '图片组件'),
(3, '商品', 'goods', '商品组件'),
(4, '品牌', 'brand', '品牌组件'),
...
INSERT INTO `#__reads_module_frame` (`frame_id`, `frame_title`, `frame_name`, `frame_explain`, `frame_structure`) VALUES
...
INSERT INTO `#__reads_navigation` (`navigation_id`, `navigation_title`, `navigation_link`, `navigation_sort`, `navigation_open_type`) VALUES
(1, '商城', '', 255, 1),
(2, '圈子', 'circle', 255, 1),
(3, '分享秀', 'shareshow', 255, 1),
(4, '品牌', 'mall/index.php?app=brand', 255, 1);
